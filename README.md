...
- 📁 **多存储后端支持**：支持 Local、阿里云 OSS、腾讯云 COS、华为云 OBS (S3 兼容)、微软 OneDrive 以及 SFTP 远程服务器。具备图形化配置界面，支持一键连接测试。
- 🛡️ **安全认证**：基于 JWT 的身份验证，支持图片验证码登录注册，具备完善的状态持久化机制。
- ⚙️ **可视化存储策略**：全新的存储策略配置页面，支持动态表单校验、配置引导及实时连通性测试。
...
- 🎨 **萌系 UI**：深度定制的 Ant Design v5 萌系皮肤，支持全局深色模式切换，极致交互体验。
...
- **框架**: React 18
- **构建工具**: Vite
- **UI 组件库**: Ant Design v5 (定制萌系皮肤 & 全局 Context 支持)
- **状态管理**: Zustand (配合 Persist 中间件实现持久化存储)
- **路由**: React Router v6
...
- **[2025-12-30] 存储策略重构**：
  - 修复了部分驱动初始化失败的 Bug。
  - 将存储策略配置从弹窗迁移至独立页面，支持动态表单。
  - 增加了存储后端连通性测试功能。
- **[2025-12-30] 系统稳定性增强**：
  - 实现了基于 Zustand Persist 的登录状态持久化，解决了页面刷新导致退出登录的问题。
  - 优化了全局消息提示（message/modal）的调用机制。
  - 修复了文件下载、分享预览及用户管理中的多处运行时错误。
- **[2025-12-29] UI/UX 优化**：
  - 全面适配 Ant Design v5，优化了深色模式下的视觉表现。
  - 改进了文件列表的加载状态及空数据展示。
